''' Origin: Particle Transformer (ParT)
...
class ParticleTransformer(nn.Module):
    ...
    def forward(self, x, msk):
        # x: (P, N, C)
        # msk: (N, P) -- real object = 0, padded = 1

        with torch.cuda.amp.autocast(enabled=self.use_amp):
            # input embedding
            x = x.masked_fill(msk.permute(1, 0).unsqueeze(-1), 0)  # (P, N, C)

            # transform
            for block in self.blocks:
                x = block(x, x_cls=None, padding_mask=msk)

            # extract class token
            cls_tokens = self.cls_token.expand(1, x.size(1), -1)  # (1, N, C)
            for block in self.cls_blocks:
                cls_tokens = block(x, x_cls=cls_tokens, padding_mask=msk)
...
class ParticleTransformerTagger(nn.Module):
    ...
    def __init__(self,
        ...
        self.evt_embed = Embed(evt_input_dim, embed_dims, activation=activation)
        ...
        self.pho_embed = Embed(pho_input_dim, embed_dims, activation=activation)
    ...
    def forward(self, evt, jet, lep, pho, msk):
        # evt, jet, lep, pho: (N, C, P)
        # msk: (N, P) -- real object = 0, padded = 1

        with torch.cuda.amp.autocast(enabled=self.use_amp):
            evt = self.evt_embed(evt)  # (P, N, C)
            jet = self.jet_embed(jet)  # (P, N, C)
            lep = self.lep_embed(lep)  # (P, N, C)
            pho = self.pho_embed(pho)  # (P, N, C)
            obj = torch.cat([evt, jet, lep, pho], dim=0)
            return self.part(obj, msk)
