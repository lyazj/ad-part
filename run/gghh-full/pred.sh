...
for ITEM in $(ls -v ../gghh/data4/); do
    python3 $HOME/work/weaver-core-dev/weaver/train.py --predict \
        ...
        --model-prefix full --tensorboard _pred_${ITEM}_full --log-file ../gghh-full/pred_${ITEM}.log \
        --predict-output ../gghh-full/pred_${ITEM}.root \
        </dev/null 2>&1 | tee ../gghh-full/pred_${ITEM}.sh.log &
done
wait
