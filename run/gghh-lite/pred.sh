...
for ITEM in $(ls -v ../gghh/data4/); do
    python3 $HOME/work/weaver-core-dev/weaver/train.py --predict \
        ...
        --model-prefix lite --tensorboard _pred_${ITEM}_lite --log-file ../gghh-lite/pred_${ITEM}.log \
        --predict-output ../gghh-lite/pred_${ITEM}.root \
        </dev/null 2>&1 | tee ../gghh-lite/pred_${ITEM}.sh.log &
done
wait
