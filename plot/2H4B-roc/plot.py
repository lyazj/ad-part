...
def compute_background_suppression(events, min_2H4BVSQCD):
    b_all, b_cut = 0.0, 0.0
    for category, category_events in events.items():
        if category == SIGNAL: continue
        b_all += ak.sum(category_events['weight'])
        b_cut += ak.sum(category_events[category_events['2H4BVSQCD'] >= min_2H4BVSQCD]['weight'])
    return b_cut / b_all

def compute_min_2H4BVSQCD(events, bs_exp, l=0.0, r=1.0):
    while l != r:
        m = (l + r) / 2
        if m == r: m = l
...
def compute_significance(events, min_2H4BVSQCD):
    s_cut, b_cut = 0.0, 0.0
    for category, category_events in events.items():
        if category == SIGNAL:
            s_cut += ak.sum(category_events[category_events['2H4BVSQCD'] >= min_2H4BVSQCD]['weight'])
        else:
            b_cut += ak.sum(category_events[category_events['2H4BVSQCD'] >= min_2H4BVSQCD]['weight'])
...
def roc(events, *args, **kwargs):
    ...
    for i, bs_exp in enumerate(np.logspace(-5, -1, n)):
        min_2H4BVSQCD = compute_min_2H4BVSQCD(events, bs_exp)
        bs = compute_background_suppression(events, min_2H4BVSQCD)
        signif = compute_significance(events, min_2H4BVSQCD)
